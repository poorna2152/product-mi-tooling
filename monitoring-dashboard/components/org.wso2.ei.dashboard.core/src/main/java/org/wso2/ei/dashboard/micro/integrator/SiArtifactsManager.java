...
public class SiArtifactsManager implements ArtifactsManager {
    private static final Logger logger = LogManager.getLogger(SiArtifactsManager.class);
    private final DataManager dataManager = DataManagerSingleton.getDataManager();
    private HeartbeatObject heartbeat = null;

    public SiArtifactsManager(HeartbeatObject heartbeat) {
    ...
    public void runFetchAllExecutorService() {
        ...
        ExecutorServiceHolder.getSiArtifactsManagerExecutorService().execute(runnable);
    }
    ...
    public String getAccessToken(String mgtApiUrl) throws ManagementApiException {
        return ManagementApiUtils.getToken(mgtApiUrl, System.getProperty("si_username"),
                System.getProperty("si_password"));
    }
